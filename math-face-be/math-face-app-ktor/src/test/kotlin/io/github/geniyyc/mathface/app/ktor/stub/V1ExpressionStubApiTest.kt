package io.github.geniyyc.mathface.app.ktor.stub
...
class V1ExpressionStubApiTest {

    @Test
    fun generateStubSuccess() = testApplication {
        application { module(MfAppSettings()) }
    ...
    @Test
    fun submitStubSuccess() = testApplication {
        application { module(MfAppSettings()) }
        val client = createClient {
            install(ContentNegotiation) {
                jackson {
    ...
    @Test
    fun generateStubBadLevel() = testApplication {
        application { module(MfAppSettings()) }
        val client = createClient {
            install(ContentNegotiation) {
                jackson {
        ...
        assertEquals(HttpStatusCode.OK, response.status)
        val body = response.body<ExpressionGenerateResponse>()
        assertEquals(ResponseResult.ERROR, body.result)
